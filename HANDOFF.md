...
- **`Groceries` category**: Added 2026-05-16 as a distinct category separate from Shopping. Covers SG supermarkets (Meidi-Ya, FairPrice, Cold Storage, Sheng Siong, etc.) and US grocers (Whole Foods, Trader Joe's, Costco, etc.). Existing transactions are not retroactively recategorised — only new imports benefit.
- **Keyword ordering within `CATEGORY_KEYWORDS`**: Specific multi-word patterns must come before broad single-word ones within the same category list — e.g. `'grab food'` and `'grabfood'` before `'grab'`, `'amazon prime'` before `'amazon'`. The categoriser uses the first match, so order determines priority.
...
- **MonthlyView transaction sort**: transactions within a month are sorted by date descending (most recent first) via `.sort((a, b) => b.date.localeCompare(a.date))`.
